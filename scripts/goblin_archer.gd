	...
	if FileAccess.file_exists(user_path):
		...
		if f_u:
			...
			if json_u.parse(txt_u) == OK and json_u.data is Dictionary:
				...
				if d_u.has("archer") and d_u["archer"] is Dictionary:
					for k in d_u["archer"]:
						a_cfg[k] = d_u["archer"][k]
						
	if a_cfg.has("ground_hips_y"):
		ground_hips_y = float(a_cfg["ground_hips_y"])
		
	for s_key in a_cfg:
		if s_key == "ground_hips_y":
			continue
		if a_cfg[s_key] is Dictionary:
			var s_dict = a_cfg[s_key]
...
func get_stance_definitions() -> Array:
	return [
		{"id": "ready", "name": "Sẵn Sàng", "shortcut": "[ Q ]"},
		{"id": "aim", "name": "Ngắm Bắn", "shortcut": "[ W ]"},
		{"id": "idle", "name": "Nghỉ", "shortcut": "[ E ]"},
		{"id": "walk", "name": "Di Chuyển", "shortcut": "[ 1 ]"},
		{"id": "run", "name": "Chạy Nhanh", "shortcut": "[ 2 ]"},
		{"id": "shoot", "name": "Bắn Cung", "shortcut": "[ 3 ]"},
		{"id": "backstep", "name": "Lùi Bước", "shortcut": "[ 4 ]"},
...
func save_stance_config() -> bool:
	...
	# 3. Save archer stances
	master_cfg["archer"] = serialize_stances()
...
func _process(delta: float) -> void:
	...
	if current_anim in ["shoot", "backstep", "hurt"]:
		if not is_in_editor:
			action_time += dt
...
func _compute_pose(anim: String, time_val: float) -> Dictionary:
	...
	match anim:
		...
		"ready": p = _compute_ready(time_val)
		"walk": p = _compute_walk(time_val)
		"run": p = _compute_run(time_val)
		"aim": p = _compute_aim(time_val)
		"shoot": p = _compute_shoot(action_time)
		"backstep": p = _compute_backstep(action_time)
